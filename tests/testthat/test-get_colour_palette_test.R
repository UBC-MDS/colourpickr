...
test_that("get_color_palette returns error message for negative limit", {
  expect_equal(get_color_palette("https://i.imgur.com/s9egWBB.jpg", 0.001, -3, TRUE), "Error: 'limit' must be a positive integer.")
})

test_that("get_color_palette returns error message for tolerance outside of range", {
  expect_equal(get_color_palette("https://i.imgur.com/s9egWBB.jpg", 1.1, 3, TRUE), "Error: 'tolerance' must be a numeric value between 0 and 1.")
})

test_that("get_color_palette returns a data frame when run with valid inputs", {
  expect_true(is.data.frame(get_color_palette("https://i.imgur.com/s9egWBB.jpg", 0.001, 3, TRUE)))
})

test_that("get_color_palette returns error message for high tolerance when force_return_limit is false", {
  result <- get_color_palette("https://i.imgur.com/s9egWBB.jpg", 1, 3, FALSE)
